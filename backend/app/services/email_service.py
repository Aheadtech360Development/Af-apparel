...
class EmailService:
    ...
    def send_order_confirmation(self, order: "Order", to_email: str) -> bool:  # type: ignore[name-defined]
        ...
        rows_html = "".join(
            f'<tr>'
            f'<td style="padding:8px 8px;font-size:12px;color:#111827;border-bottom:1px solid #f3f4f6;word-break:break-word">'
            f'{item.product_name}</td>'
            f'<td style="padding:8px 8px;font-size:12px;color:#374151;border-bottom:1px solid #f3f4f6;white-space:nowrap">'
            f'{item.color or "—"} / {item.size or "—"}</td>'
            f'<td style="padding:8px 8px;font-size:12px;font-weight:700;text-align:center;'
            f'border-bottom:1px solid #f3f4f6;white-space:nowrap">{item.quantity}</td>'
            f'<td style="padding:8px 8px;font-size:12px;text-align:right;'
            f'border-bottom:1px solid #f3f4f6;white-space:nowrap">${float(item.unit_price):.2f}</td>'
            f'<td style="padding:8px 8px;font-size:12px;font-weight:700;text-align:right;'
            f'border-bottom:1px solid #f3f4f6;white-space:nowrap">${float(item.line_total):.2f}</td>'
            f'</tr>'
        ...
        content_html = (
            ...
            f'<th style="padding:8px 8px;color:#fff;text-align:left;font-size:11px;'
            f'font-weight:700;letter-spacing:.06em;width:40%">PRODUCT</th>'
            f'<th style="padding:8px 8px;color:#fff;text-align:left;font-size:11px;'
            f'font-weight:700;letter-spacing:.06em;width:28%">VARIANT</th>'
            f'<th style="padding:8px 8px;color:#fff;text-align:center;font-size:11px;'
            f'font-weight:700;letter-spacing:.06em;width:10%">QTY</th>'
            f'<th style="padding:8px 8px;color:#fff;text-align:right;font-size:11px;'
            f'font-weight:700;letter-spacing:.06em;width:11%">UNIT</th>'
            f'<th style="padding:8px 8px;color:#fff;text-align:right;font-size:11px;'
            f'font-weight:700;letter-spacing:.06em;width:11%">TOTAL</th>'
            f'</tr></thead>'
